...
extern uint32_t hall_sens_read_full();
...
void TempHallstr(char* string,int maxlenstr)
{
    snprintf(string,maxlenstr,"<br>Temp:%.2f<br>Magnetic:%d %d",(temprature_sens_read()-32)/1.8,hall_sens_read(),hall_sens_read_full());
            ...
            if(strstr(first_line, "GET / ")) 
                {
                 ...
                 netconn_write(conn, HallTmp, strlen(HallTmp), NETCONN_NOCOPY);
                }
...
void app_main()
    ...
    WriteStringToOled((char*)"FraAnt 18",10,1,1,0);
    WriteMiniStringToOled((char*)"The Best Family.....",1,7,0,0);
    WriteStringToOled((char*)"ESP8266EX",14,3,0,1);
#endif
    ...
    int8_t pw=78;
    ESP_ERROR_CHECK(esp_wifi_set_max_tx_power(pw));//>77 max level<8 min level
    //Levels -4,8,20,28,34,44,52,60,68,74,76,78(19.5dbm)    
    ESP_LOGI(TAG, "Wifi power set to %d.", pw);
    ESP_ERROR_CHECK(esp_wifi_get_max_tx_power(&pw));
    ESP_LOGI(TAG, "Wifi power get is %d.", pw);
    ...
    xTaskCreate(&http_server, "http_server", 8192, NULL, 3, NULL);
    ...
    while(1)
        {
         ...
         if(seconds>TTL_PACKET_TIME)
            {
              xTaskCreate(FrKeepAlive, "udp_keepalive", 2048, NULL, 5, NULL);
              seconds=0;
            }
         if(ERRORS>0)
                {
                 printf("EspRestart, FrError:%d",ERRORS);
                 esp_restart();
                }//mio watchdog.. riavvio

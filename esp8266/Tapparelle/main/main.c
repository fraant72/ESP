...
#define UDP_PORT 27000
#define UDP_FIX_LEN 9
#define UDPTICKSET 6//numero di secondi dopo il quale accetto pacchetti duplicati
#define CMD_STOP "FSTOP01F"//cmd STOP
#define CMD_OPEN "FOPEN02F"//cmd OPEN 
#define CMD_CLOS "FCLOS03F"//cmd CLOSE 
#define CMD_OPPO "FOPNP04F"//Cmd OPEN poco
#define SECPAUSA 5//secondi di pausa
#define OPENSEC  25//secondi necessari per aprire
#define CLOSSEC  23//secondi necessari per aprire

char UdpLastCmd[UDP_FIX_LEN+1]="";//ultimo comando UDP
...
// actual relay status
uint8_t opening=0;//apertura in corso
uint8_t closing=1;//chiusura in corso

int relay1on=0;int relay2on=0;//conto da quanti secondi consecutivi i relay sono accesi
uint8_t udptick=0;//tick timer per evitarte comandi ripetuti
uint8_t stop=0;//se maggiore di 0 vengono spenti i relay
...
void spegnitutto(void)
{
     ...
     gpio_set_level(GPIO_OUTPUT_1, 0);//spengo la discesa
     gpio_set_level(GPIO_OUTPUT_0, 0);//accendo la salita

}

//open tapparella
static void CloseTap_task(void *pvParameters)
{
    closing=1;
    spegnitutto();   
    uint16_t timer=OPENSEC*10;
 
    while(timer>0)
        {
         gpio_set_level(GPIO_OUTPUT_1, 1);//accendo la Discesa
         vTaskDelay((98 / portTICK_PERIOD_MS));
         timer--;
         if(timer%10==0)
                {
                 ESP_LOGI(TAG, "Close remaining sec %d", timer/10);
                }
         if(timer%110==0)//ogni dieci secondi
            {
            //pausa di sicurezza in discesa
             gpio_set_level(GPIO_OUTPUT_1, 0);//accendo la Discesa
             for(int i=0;i<(SECPAUSA*10);i++)
                {
                 if(i%10==0){ESP_LOGI(TAG, "Security Pause  sec %d", i/10);}
                 vTaskDelay((98 / portTICK_PERIOD_MS));
                 if(stop>0){goto RAPIDEXIT;}
                }
             gpio_set_level(GPIO_OUTPUT_1, 1);//accendo la Discesa
...
//open tapparella
static void OpenTap_task(void *pvParameters)
{
    opening=1;
    spegnitutto();   
    uint16_t timer=OPENSEC*10;
    ...
    while(timer>0)
        {
         gpio_set_level(GPIO_OUTPUT_0, 1);//accendo la salita
         vTaskDelay((98 / portTICK_PERIOD_MS));
         timer--;
         if(timer%10==0){ESP_LOGI(TAG, "Opening remaining sec %d", timer/10);}
         if(stop>0)
            {
            spegnitutto();
            break;
...
static void udp_server_task(void *pvParameters)
    ...
    while (1) 
        ...
        ESP_LOGI(TAG, "UDP Socket created, port:%d",UDP_PORT);
        ...
        while (1) 
            ...
            // Data received
                ...
                if (sourceAddr.sin6_family == PF_INET) 
                    ...
                    ESP_LOGI(TAG, "UDP Received %d bytes from %s, fix_len:%d:", len, addr_str,UDP_FIX_LEN);
                    ESP_LOGI(TAG, "%s", rx_buffer);

                    if(len!=UDP_FIX_LEN){continue;}//accetto solo comandi della lunghezza fissa
                    if(rx_buffer[0]!='F'){continue;}//il pacchetto deve iniziare con la F maiuscola
                    if(rx_buffer[UDP_FIX_LEN-2]!='F'){continue;}//il pacchetto deve iniziare con la F maiuscola
                    
                    //evito reply in n secondi
                    if(strstr(UdpLastCmd,rx_buffer)!=0)//cerco i duplicati
                        {
                         if(udptick>0)
                            {
                             ESP_LOGI(TAG, "ReplyCmdUDP, ignoring tick:%d",udptick);
                             continue;
                            }
                        }

                    udptick=UDPTICKSET;
                    memcpy(UdpLastCmd,rx_buffer,UDP_FIX_LEN);
                    //Eseeguo i comandi 
                   
                    if(strstr(rx_buffer,CMD_STOP)!=0)
                        {
                         ESP_LOGI(TAG, "cmd stop");
                         stop=1;//spegne tutti i relay
                         //Stop Command

                        } 

                    if(strstr(rx_buffer,CMD_OPEN)!=0)
                        {
                         ESP_LOGI(TAG, "cmd open");
                         stop=1;
                         vTaskDelay((490 / portTICK_PERIOD_MS));//attendo che eventuali thread muoiano
                         stop=0;
                         xTaskCreate(OpenTap_task, "cmd_open", 1024, NULL, 5, NULL);
                         //Open Command

                        } 

                    if(strstr(rx_buffer,CMD_CLOS)!=0)
                        {
                         stop=1;
                         vTaskDelay((490 / portTICK_PERIOD_MS));//attendo che eventuali thread muoiano
                         stop=0;
                         xTaskCreate(CloseTap_task, "cmd_close", 1024, NULL, 5, NULL);
                    ...
                    if(strstr(rx_buffer,CMD_OPPO)!=0)
                        {
                         stop=0;
                         ESP_LOGI(TAG, "cmd close Poco");
                         //Close Poco Command
...
static void gpio_task_example(void *arg)
    ...
    for (;;) 
        ...
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) 
            ...
            if((st==1)&&(io_num==GPIO_INPUT_IO_1))
                {
                 ESP_LOGI(TAG, "cmd Open-Push");
                 stop=1;
                 vTaskDelay((490 / portTICK_PERIOD_MS));//attendo che eventuali thread muoiano
                 stop=0;
                 xTaskCreate(OpenTap_task, "cmd_open", 1024, NULL, 5, NULL);
...
void app_main()
    ...
    xTaskCreate(&http_server, "http_server", 3072, NULL, 3, NULL);
    xTaskCreate(&udp_server_task, "udp_server", 3072, NULL, 3, NULL);
    ...
    while(1)
        ...
        if (tv.tv_sec!=lastsec)
            ...
            //stampa icone relative ai led su OLED
                if(opening==1)
                    {
